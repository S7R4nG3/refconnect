...
// leadFrames is the number of silence-only frames emitted before the first
// DPRS segment. Gives the gateway a chance to open its "stream" state
// before any slow-data is interpreted.
const leadFrames = 2
...
func Send(refl protocol.Reflector, srcCall, rpt1, rpt2, dprsSentence string) error {
	if refl == nil {
		return fmt.Errorf("beacon: no reflector")
	}
	if refl.State() != protocol.StateConnected {
		return fmt.Errorf("beacon: reflector not connected")
	}
	...
	if err := refl.SendHeader(hdr); err != nil {
		return fmt.Errorf("beacon: send header: %w", err)
	}

	// Lead-in silence frames (no GPS slow data yet).
	seq := uint8(0)
	for i := 0; i < leadFrames; i++ {
		f := silenceFrame(seq, false)
		if err := refl.SendFrame(f); err != nil {
			return fmt.Errorf("beacon: lead frame: %w", err)
		}
		seq = (seq + 1) % (dstar.MaxSeq + 1)
	}

	// DPRS payload frames — slow data carries the encoded sentence.
	...
	for i, sd := range slowFrames {
		...
		f.SlowData = sd
		if err := refl.SendFrame(f); err != nil {
			return fmt.Errorf("beacon: data frame: %w", err)
		}
		seq = (seq + 1) % (dstar.MaxSeq + 1)
	}
	return nil
